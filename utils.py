...
class AdbHelper(object):
    def __init__(self, device_name, enable_switch_to_root=True):
    ...
    def run_and_return_output(self, adb_args, stdout_file=None, log_output=True):
        if self.device_name:
            adb_args = [self.adb_path] + '-s' + self.device_name + adb_args
        else:
    ...
    def remount(self):
        if not self.enable_switch_to_root:
            log_debug("can not remount on an unrooted device")
            return False
        result, stdoutdata = self.run_and_return_output(['remount'])
        return result and 'succeeded' in stdoutdata
    ...
    def mkdir(self, dir_parent_path, dir_name):
        result, stdoutdata = self.run_and_return_output(['shell', 'ls', dir_parent_path], log_output=False)
        if not result:
            return False
        if dir_name in stdoutdata:
            return True
        else:
            result, stdoutdata = self.run_and_return_output(['shel', 'cd', dir_parent_path, '&&', 'mkdir', dir_name])
        return result
